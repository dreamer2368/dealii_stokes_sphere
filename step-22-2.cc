/* ---------------------------------------------------------------------
 * $Id: step-22.cc 30526 2013-08-29 20:06:27Z felix.gruber $
 *
...
namespace Step22
{
                              ...
                              const unsigned int component) const
  {
    ...
    if (component == dim-1)
		{
      if(p[dim-1] == -half_length)
        return 1;
/*
			else if(p[0] == 0 && p[dim-1] < 0)
				return 1;
*/
		}
    return 0;
  ...
  template <int dim>
  class SphereBoundaryValues : public Function<dim>
  {
  public:
    SphereBoundaryValues () : Function<dim>(dim+1) {}
  ...
  template <int dim>
  double
  SphereBoundaryValues<dim>::value (const Point<dim>  &p,
                              const unsigned int component) const
  {
    Assert (component < this->n_components,
            ExcIndexRange (component, 0, this->n_components));
  ...
  template <int dim>
  void
  SphereBoundaryValues<dim>::vector_value (const Point<dim> &p,
                                     Vector<double>   &values) const
  {
    for (unsigned int c=0; c<this->n_components; ++c)
      values(c) = SphereBoundaryValues<dim>::value (p, c);
  }
...
*/
  ...
  template <int dim>
  class R_linear : public Function<dim>
  {
  public:
    R_linear () : Function<dim>() {}
  ...
  template <int dim>
  double
  R_linear<dim>::value (const Point<dim>  &p,
                             const unsigned int /*component*/) const
  {
    return p[dim-2];
  }
  ...
  template <int dim>
  void
  R_linear<dim>::value_list (const std::vector <Point<dim> > &points,
                                    std::vector<double>   &values) const
  {
		const unsigned int n_points = points.size();
    for (unsigned int i=0; i<n_points; ++i)
      values[i] = R_linear<dim>::value (points[i]);
  }

  template <int dim>
  class R_square : public Function<dim>
  {
  public:
    R_square () : Function<dim>() {}
  ...
  template <int dim>
  double
  R_square<dim>::value (const Point<dim>  &p,
                             const unsigned int /*component*/) const
  {
    return p[dim-2]*p[dim-2];
  }


  template <int dim>
  void
  R_square<dim>::value_list (const std::vector <Point<dim> > &points,
                                    std::vector<double>   &values) const
  {
		const unsigned int n_points = points.size();
    for (unsigned int i=0; i<n_points; ++i)
      values[i] = R_square<dim>::value (points[i]);
  }
  ...
  void StokesProblem<dim>::assemble_system ()
  {
    ...
    std::vector<Vector<double> >      rhs_values (n_q_points,
		...
		const R_linear<dim> r_linear;
		std::vector<double> r_linear_values (n_q_points);

		const R_square<dim> r_square;
		std::vector<double> r_square_values (n_q_points);
    ...
    std::vector<Tensor<1,dim> >					 spu0 (dofs_per_cell);
    ...
    for (; cell!=endc; ++cell)
      {
        ...
        right_hand_side.vector_value_list(fe_values.get_quadrature_points(),
                                          rhs_values);
				r_linear.value_list (fe_values.get_quadrature_points(), r_linear_values);
				r_square.value_list (fe_values.get_quadrature_points(), r_square_values);
            ...
            for (unsigned int k=0; k<dofs_per_cell; ++k)
              {
                ...
                phi_p[k]         = fe_values[pressure].value (k, q);
								...
								spu0[k]					 = 0.5 * (fe_values[velocities].gradient (k, q)[0]
																				 + transpose( fe_values[velocities].gradient (k, q) )[0]);
              }
            ...
            for (unsigned int i=0; i<dofs_per_cell; ++i)
              {
                for (unsigned int j=0; j<dofs_per_cell; ++j)
                  {
                    local_matrix(i,j) += (2 * symgrad_phi_u[i] * symgrad_phi_u[j] * r_linear_values[q]
                                          + 2 * ( r_linear_values[q] == 0 ? symgrad_phi_u[i][0][0] * phi_u[j][0] + phi_u[i][0] * symgrad_phi_u[j][0][0] : phi_u[i][0] * phi_u[j][0] / r_linear_values[q] )
                                          - div_phi_u[i] * phi_p[j] * r_linear_values[q]
																					- phi_u[i][0] * phi_p[j]
                                          - phi_p[i] * div_phi_u[j] * r_linear_values[q]
																					- phi_p[i] * phi_u[j][0]
                                          + phi_p[i] * phi_p[j])
                                         * fe_values.JxW(q);
                  }
...
*/
  ...
  void StokesProblem<dim>::solve ()
  {
    ...
    QGauss<dim-1>   quadrature_formula(degree+2);
    const unsigned int   n_q_points      = quadrature_formula.size();
    FEFaceValues<dim> fe_face_values (fe, quadrature_formula,
                             ...
                             update_gradients);
    std::vector<Tensor<2,dim> > vel_grad (n_q_points);
    std::vector< Point<dim> > face_normal (n_q_points);
    std::vector< double >     press_q (n_q_points);

		const R_linear<dim> r_linear;
		std::vector<double> r_linear_values (n_q_points);

    Tensor<1,dim> integral( {0, 0} );
    const FEValuesExtractors::Vector velocities (0);
    const FEValuesExtractors::Scalar pressure (dim);
    typename DoFHandler<dim>::active_cell_iterator
      cell = dof_handler.begin_active(),
	   ...
	   ++f)
        if( cell->face(f)->at_boundary() &&
            cell->face(f)->boundary_indicator() == 1 )
          {
            ...
            fe_face_values[velocities].get_function_gradients (solution, vel_grad);
            fe_face_values[pressure].get_function_values (solution, press_q);
            face_normal = fe_face_values.get_normal_vectors();
						r_linear.value_list (fe_face_values.get_quadrature_points(), r_linear_values);
            for(unsigned int q=0; q<fe_face_values.n_quadrature_points; ++q)
            {

              integral += ( face_normal[q] * press_q[q]
                          - face_normal[q] * ( vel_grad[q] + transpose(vel_grad[q]) ) )
                          * 2 * PI * r_linear_values[q] * fe_face_values.JxW(q);
//std::cout<<r_linear_values[q]<<std::endl;
            }
          }
    integral /= 6*PI;
    std::cout<<"drag coefficient = ("<<integral[0]<<", "<<integral[1]<<")"<<std::endl;
      
      
      double max_pressure = 0;
      double min_pressure = 0;
      
      for( int i=0; i<solution.block(1).size(); i++)
      {
          if (max_pressure<solution.block(1)[i] ) {
              max_pressure = solution.block(1)[i];
          }
          if (min_pressure>solution.block(1)[i]) {
              min_pressure = solution.block(1)[i];
          }
      }
      std::cout<<"max p = "<<max_pressure<<", min p = "<<min_pressure<<std::endl;
      
      if ( !additional_data.is_open() ) {
          std::ostringstream filename;
          filename << "exterior size-"
          << Utilities::int_to_string (cylinder_length, 2)
          << ".txt";
          additional_data.open(filename.str().c_str());
          additional_data<<"N_dofs	min_p	max_p	drag_r	drag_z"<<std::endl;
      }
      additional_data<<dof_handler.n_dofs()<<"  "<<min_pressure<<"  "<<max_pressure<<"  "<<integral[0]<<"   "<<integral[1]<<std::endl;
  }
  ...
  StokesProblem<dim>::refine_mesh ()
  {
    ...
    std::ofstream out ("grid-refined.eps");
    GridOut grid_out;
  ...
  { 
  	...
  	{
    	std::string filename = "sphere2D.ucd";
    ...
    for (; cell!=endc; ++cell)
	   			...
	   			++f)
      {
				...
				if ( face_center[0] == cylinder_radius ||
						 face_center[0] == 0 ||
      	     face_center[1] == half_length )
      	  cell->face(f)->set_all_boundary_indicators(0);
				else if( face_center[1] == -half_length )
					cell->face(f)->set_all_boundary_indicators(2);
				else if( face_center.norm() < ball_radius )
					cell->face(f)->set_all_boundary_indicators(1);
/*
				if ( face_center[0] == cylinder_radius ||
      	     face_center[1] == cylinder_radius )
      	  cell->face(f)->set_all_boundary_indicators(0);
				else if( face_center[1] == -cylinder_radius ||
						 		 face_center[0] == 0 )
				{
					unsigned int b = ( face_center[1] == -cylinder_radius ? 2 : ( face_center[1] < 0 ? 2 : 0) );
					cell->face(f)->set_all_boundary_indicators(b);
				}
				else if( face_center.norm() < ball_radius )
					cell->face(f)->set_all_boundary_indicators(1);
*/
    ...
    const HalfHyperBallBoundary<2> boundary_description(center,ball_radius);
  
    triangulation.set_boundary (1, boundary_description);
  
//      (cylinder_radius < 15 ? triangulation.refine_global(1) : triangulation.refine_global(2) );
    ...
    for (; cell!=endc; ++cell)
      for (unsigned int v=0;
          ...
          ++v)
        if ( std::fabs( cell->vertex(v).norm() - ball_radius ) < tol )
        {
    ...
    // and matrices, assemble, solve and create output:
      ...
      std::cout<<"Exterior Boundary size = "<<cylinder_length<<std::endl;
