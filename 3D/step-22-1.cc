                              ...
                              const unsigned int component) const
  {
    ...
    if (component == dim-1)
    {
      if(p[dim-1] == -half_length)
        ...
        return ( std::fabs( center.distance (p) - ball_radius ) < 1e-10  ? 0 : 1);
    }
    return 0;
  }
...
*/
  ...
  void StokesProblem<dim>::solve ()
  {
    ...
    integral /= 6*PI;
    std::cout<<"drag coefficient = ("<<integral[0]<<", "<<integral[1]<<", "<<integral[2]<<")"<<std::endl;
  }
  ...
  { 
    ...
    {
      std::string filename = "grid-1.ucd";
	...
	if ( face_center.norm() < ball_radius )
        {
	  cell->face(f)->set_all_boundary_indicators(1);
        }
        else if ( face_center[2] == half_length ||
                  abs(face_center[0]) == cylinder_radius ||
                  abs(face_center[1]) == cylinder_radius )
          cell->face(f)->set_all_boundary_indicators(0);
	else if ( face_center[2] == -half_length )
          cell->face(f)->set_all_boundary_indicators(2);
      }
  
    const HyperBallBoundary<3> boundary_description(center,ball_radius);
    ...
    triangulation.execute_coarsening_and_refinement ();
    std::ofstream out ("grid-initial.eps");
    GridOut grid_out;
    grid_out.write_eps (triangulation, out);
